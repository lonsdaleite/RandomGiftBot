...
async def notify_user(user):
    ...
    dml_actions.add_notification_log(user.user_id, user.notification_time, "1")
    config.logger.info("Notification sent to " + str(user.tg_id))


async def notify():
    user_id_list = select_actions.get_user_id_by_time(time.strftime("%H:%M", time.localtime()))
